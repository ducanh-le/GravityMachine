...
function roundingSolutionNew23!(vg::Vector{tGenerateur}, k::Int64, c1::Array{Int,1}, c2::Array{Int,1}, d::tListDisplay)
    ...
    nbvar = length(vg[k].sInt.x)
    nbgen = size(vg,1)
    lstIdxFrac =(Int64)[]   #liste ID des variables fractionnaires
    ...
    #pour arrondir vers le milieu de sPrj et sRel
    z3 = (vg[k].sPrj.y[1] + vg[k].sRel.y[1])/2
    z4 = (vg[k].sPrj.y[2] + vg[k].sRel.y[2])/2
...
#        @printf("\n")
        ...
        for iFrac in [i for i=1:nbvar if vg[k].sInt.x[i] == -1]
            ...
            if inCone1VersZ(pCour, pSuiv, pPrec, pM)
                ...
                distL1 = abs(z3-pM.x) + abs(z4-pM.y)
                if distL1 < distMin
...
# ==============================================================================
# arrondi la solution correspondant au generateur (pas d'historique donc)
# version avec voisinage et selection d'un voisin selon distance L1, soit avec generateur soit avec le milieu de generateur et sPrj
# le voisin dans les cones est priorisé
function roundingSolutionNew25!(vg::Vector{tGenerateur}, k::Int64, c1::Array{Int,1}, c2::Array{Int,1}, d::tListDisplay)
    ...
    for i= 1:nbVarNonEntiere
        ...
        distMinInCone = Inf; distMinOutCone = Inf; ind = -1; val = -1; inConeFound = false
        # voisinage a 1 variable sur toutes les variables
        for iFrac in [i for i=1:nbvar if vg[k].sInt.x[i] == -1]

            # defalque la contribution fractionnaire de la variable aux objectifs => pose x[iFrac]=0
            pM0 = tPoint( z1 - c1[iFrac] * vg[k].sPrj.x[iFrac] , z2 - c2[iFrac] * vg[k].sPrj.x[iFrac])
            # ajoute la contribution entiere de la variable aux objectifs => pose x[iFrac]=1
            pM1 = tPoint( z1 - c1[iFrac] * vg[k].sPrj.x[iFrac] + c1[iFrac], z2 - c2[iFrac] * vg[k].sPrj.x[iFrac] + c2[iFrac])

            if inCone1VersZ(pCour, pSuiv, pPrec, pM0)
                distL1 = abs(vg[k].sRel.y[1]-pM0.x) + abs(vg[k].sRel.y[2]-pM0.y)
                #distL1 = abs(z3-pM0.x) + abs(z4-pM0.y)
                if inCone2Vers0(pC, pSuiv, pPrec, pM0)
                    inConeFound = true
                    if distL1 < distMinInCone
                        distMinInCone = distL1; ind = iFrac; val = 0
                    end
                elseif !inConeFound
                    if distL1 < distMinOutCone
                        distMinOutCone = distL1; ind = iFrac; val = 0
                    end
                end
            end
            
            distL1 = abs(vg[k].sRel.y[1]-pM1.x) + abs(vg[k].sRel.y[2]-pM1.y)
            #distL1 = abs(z3-pM1.x) + abs(z4-pM1.y)
            if inCone2Vers0(pC, pSuiv, pPrec, pM1)
                inConeFound = true
                if distL1 < distMinInCone
                    distMinInCone = distL1; ind = iFrac; val = 1
                end
            elseif !inConeFound
                if distL1 < distMinOutCone
                    distMinOutCone = distL1; ind = iFrac; val = 1
                end
            end
    ...
    @printf("→ #round : %4d → [ %5d , %5d ] %s ", nbVarNonEntiere, vg[k].sInt.y[1], vg[k].sInt.y[2], inConeFound ? "inCone " : "outCone")
